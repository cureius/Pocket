...
class TransactionsViewModel @Inject constructor(
    ...
    private fun getTransactionsCreatedOnMonthForAccounts(
        transactionOrder: TransactionOrder, firstDayOfMonth: Long, lastDayOfMonth: Long
    ) {
        getTransactionsCreatedOnCurrentMonthForAccountsJob?.cancel()
        getTransactionsCreatedOnCurrentMonthForAccountsJob =
            accountUseCases.getAccounts().onEach { accounts ->
                transactionUseCases.getTransactionsForDateRange(
                    transactionOrder, firstDayOfMonth, lastDayOfMonth
                ).onEach { transactions ->
                    _state.value = state.value.copy(
                        transactionsOnCurrentMonthForAccounts = transactions.filter {
                            ((it.account)?.toInt()
    ...
    private fun getTransactionsForAccounts(transactionOrder: TransactionOrder) {
        ...
        getTransactionsForAccountsJob = accountUseCases.getAccounts().onEach { accounts ->
            transactionUseCases.getTransactions(transactionOrder).onEach { transactions ->
                _state.value = state.value.copy(
    ...
    private fun getTransactionsForDateRange(
        transactionOrder: TransactionOrder, start: Long, end: Long
    ) {
